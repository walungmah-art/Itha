    ...
    if charged_card:
        ...
        for r in results:
            ...
            if r['status'] == 'LIVE':
                ...
                # Extract specific decline code if present
                elif 'incorrect_cvc' in decline_msg.lower():
                    decline_msg = 'Incorrect CVC'
                elif 'insufficient_funds' in decline_msg.lower():
                    decline_msg = 'Insufficient funds'
                elif 'do_not_honor' in decline_msg.lower():
                    decline_msg = 'Do not honor'
                response += f"    {status_emoji} <code>{card_display}</code>\n"
        ...
        # Show LIVE cards summary if any
        ...
        if captcha_count > 0:
            response += f" | 🧩 𝗖𝗮𝗽𝘁𝗰𝗵𝗮: {captcha_count}"
        if three_ds_count > 0:
